...
/** Alternate Claude Code profiles (`CLAUDE_CONFIG_DIR=~/.claude-<name>`) are targeted
 * like the default profile: every `.claude-*` directory's CLAUDE.md carries the
 * section, created if absent (owner ruling 2026-08-23 - previously profiles opted in
 * by already carrying the marker). The directory itself is the evidence of a profile;
 * only the user creates those. A symlink whose target is a directory is a profile
 * too - CLAUDE_CONFIG_DIR follows it - so entries are stat'ed, not judged by their
 * Dirent type. */
function claudeProfileTargets(home: string): ConfigTarget[] {
  ...
  for (const entry of readHomeEntries(home)) {
    ...
    try {
      if (!statSync(join(home, entry.name)).isDirectory()) continue;
    } catch {
      continue; // dangling symlink or unreadable entry - not a usable profile
    }
    targets.push({ path: join(home, entry.name, "CLAUDE.md"), kind: "block" });
  }
  return targets;
}
 ...
 * every alternate Claude profile directory. `home` is injectable for tests. */
