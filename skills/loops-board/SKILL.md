...
**Descriptive vs prescriptive.** A *descriptive* item, one documenting behavior that
already exists (runbooks, references, "how it works" notes), must declare `depends-on`
for every implementation item it describes. A *prescriptive* item, one driving what
gets built (specs, plans, ADRs), declares no such dependency.
...
5. `tested`/`delivered` items move to `for-delivery/`, `accepted`/`dropped` to
   `archive/` (+ `ARCHIVE.md` row); `bun run sync` performs the moves, so just set the
